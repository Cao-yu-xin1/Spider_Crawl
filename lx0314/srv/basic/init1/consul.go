...
func ConsulInit() error {
	...
	consulConfig.Address = fmt.Sprintf("%s:%d", nacos.GlobalConfig.Consul.Host, nacos.GlobalConfig.Consul.Port)
	var err error
	...
	serviceID = fmt.Sprintf("%s-%d", nacos.GlobalConfig.Consul.ServiceName, time.Now().Unix())
	checkID := fmt.Sprintf("%s-health", serviceID)
	registration := &api.AgentServiceRegistration{
		ID:      serviceID,
		Name:    nacos.GlobalConfig.Consul.ServiceName,
		Address: "localhost",
		Port:    nacos.GlobalConfig.Consul.ServicePort,
		Checks: []*api.AgentServiceCheck{
			{
				CheckID:                        checkID,
				Name:                           "TTL Health Check",
				TTL:                            fmt.Sprintf("%ds", nacos.GlobalConfig.Consul.TTL),
	...
	go func() {
		ticker := time.NewTicker(time.Duration(nacos.GlobalConfig.Consul.TTL/2) * time.Second)
		defer ticker.Stop()
		for range ticker.C {
